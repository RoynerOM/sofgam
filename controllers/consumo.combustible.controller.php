<?php
class ConsumoCombustibleController
{
    /*=============================================
	Creación accion
	=============================================*/
    public function create()
    {
        if (isset($_POST["tipo"])) {
            ...
            if (
				...
				Solicitud a la API
				=============================================*/
                $url = "consumos_combustibles?token=" . $_SESSION["admin"]->token_user;
                $method = "POST";
                $fields = $data;
                ...
                if ($response->status == 200) {
                    ...
                    /* $url = "users?id=" . $id . "&nameId=id_user&token=" . $_SESSION["admin"]->token_user . "&table=users&suffix=user";
                        ...
                        if ($response->status == 200) {
                            echo '<script>
								...
								fncSweetAlert("Exitosamente", "El registro fue creado exitosamente.", "/consumo_combustible");
							</script>';
                        } */
                    /* } else { */
                    /* echo '<script>
                    ...
                    echo '<script>
								...
								matPreloader("off");
								fncSweetAlert("close", "", "");
								fncSweetAlert("success", "El registro fue creado exitosamente.", "/consumo_combustible");
							</script>';
                }
            } else {
                echo '<script>
    ...
    {
        if (isset($_POST["idConsumoCombustible"])) {
            echo '<script>
				matPreloader("on");
				fncSweetAlert("loading", "Cargando...", "");
			</script>';

            if ($id == $_POST["idConsumoCombustible"]) {
                $url = "consumos_combustibles?linkTo=id&equalTo=" . $id;
                $method = "GET";
                $fields = array();
                $response = CurlController::request($url, $method, $fields);
                if ($response->status == 200) {
                    ...
                    if (
						...
						Agrupamos la información 
						=============================================*/
                        $data = "tipo=".trim($_POST["tipo"])."&enero=".trim($_POST["enero"])."&febrero=".trim($_POST["febrero"])."&marzo=".trim($_POST["marzo"])."&abril=".trim($_POST["abril"])."&mayo=".trim($_POST["mayo"])."&junio=".trim($_POST["junio"])."&julio=".trim($_POST["julio"])."&agosto=".trim($_POST["agosto"])."&septiembre=".trim($_POST["septiembre"])."&octubre=".trim($_POST["octubre"])."&noviembre=".trim($_POST["noviembre"])."&diciembre=".trim($_POST["diciembre"])."&total=".trim($_POST["total"]);
                        /*=============================================
						Solicitud a la API
						=============================================*/
                        $url = "consumos_combustibles?id=" . $id . "&nameId=id&token=" . $_SESSION["admin"]->token_user;
                        $method = "PUT";
                        $fields = $data;
                        $response = CurlController::request($url, $method, $fields);
                        /*=============================================
