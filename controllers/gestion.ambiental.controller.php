...
class SistemaTratamientoController
{
    ...
    public function create()
    {
        if (isset($_POST["nombre"])) {
            ...
            if (
				...
				Solicitud a la API
				=============================================*/
                $url = "table?token=" . $_SESSION["admin"]->token_user;
                $method = "POST";
                $fields = $data;
                ...
                if ($response->status == 200) {
                    ...
                    /* $url = "users?id=" . $id . "&nameId=id_user&token=" . $_SESSION["admin"]->token_user . "&table=users&suffix=user";
                        ...
                        if ($response->status == 200) {
                            echo '<script>
								...
								fncSweetAlert("Exitosamente", "El registro fue creado exitosamente.", "/gestion_ambiental");
							</script>';
                        } */
                    /* } else { */
                    /* echo '<script>
                    ...
                    echo '<script>
								...
								matPreloader("off");
								fncSweetAlert("close", "", "");
								fncSweetAlert("success", "El registro fue creado exitosamente.", "/gestion_ambiental");
							</script>';
                }
            } else {
                echo '<script>
    ...
    {
        if (isset($_POST["idGestionAmbiental"])) {
            echo '<script>
				matPreloader("on");
				fncSweetAlert("loading", "Cargando...", "");
			</script>';

            if ($id == $_POST["idGestionAmbiental"]) {
                $url = "table?linkTo=id&equalTo=" . $id;
                $method = "GET";
                $fields = array();
                $response = CurlController::request($url, $method, $fields);
                if ($response->status == 200) {
                    ...
                    if (
						...
						Solicitud a la API
						=============================================*/
                        $url = "table?id=" . $id . "&nameId=id&token=" . $_SESSION["admin"]->token_user;
                        $method = "PUT";
                        $fields = $data;
                        $response = CurlController::request($url, $method, $fields);
                        /*=============================================
