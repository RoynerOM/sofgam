<?php
class AhorroConsumoAguaController
{
    /*=============================================
	Creación accion
	=============================================*/
    public function create()
    {
        if (isset($_POST["mes"])) {
            ...
            if (
				...
				Solicitud a la API
				=============================================*/
                $url = "ahorros_consumos_agua?token=" . $_SESSION["admin"]->token_user;
                $method = "POST";
                $fields = $data;
                ...
                if ($response->status == 200) {
                    ...
                    /* $url = "users?id=" . $id . "&nameId=id_user&token=" . $_SESSION["admin"]->token_user . "&table=users&suffix=user";
                        ...
                        if ($response->status == 200) {
                            echo '<script>
								...
								fncSweetAlert("Exitosamente", "El registro fue creado exitosamente.", "/ahorro_consumo_agua");
							</script>';
                        } */
                    /* } else { */
                    /* echo '<script>
                    ...
                    echo '<script>
								...
								matPreloader("off");
								fncSweetAlert("close", "", "");
								fncSweetAlert("success", "El registro fue creado exitosamente.", "/ahorro_consumo_agua");
							</script>';
                }
            } else {
                echo '<script>
    ...
    {
        if (isset($_POST["idAhorroConsumoAgua"])) {
            echo '<script>
				matPreloader("on");
				fncSweetAlert("loading", "Cargando...", "");
			</script>';

            if ($id == $_POST["idAhorroConsumoAgua"]) {
                $url = "ahorros_consumos_agua?linkTo=id&equalTo=" . $id;
                $method = "GET";
                $fields = array();
                $response = CurlController::request($url, $method, $fields);
                if ($response->status == 200) {
                    ...
                    if (
						...
						Solicitud a la API
						=============================================*/
                        $url = "ahorros_consumos_agua?id=" . $id . "&nameId=id&token=" . $_SESSION["admin"]->token_user;
                        $method = "PUT";
                        $fields = $data;
                        $response = CurlController::request($url, $method, $fields);
                        /*=============================================
